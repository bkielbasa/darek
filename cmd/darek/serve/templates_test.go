...
func TestIndexRendersWithChrome(t *testing.T) {
	...
	s, err := New(nil, nil, nil, a, &OIDC{}, nil, nil, "", nil)
	if err != nil {
		t.Fatalf("new: %v", err)
	}
...
func TestWhatsAppRendersWithChrome(t *testing.T) {
	...
	s, err := New(nil, nil, nil, a, &OIDC{}, fakeWhatsAppManager{}, nil, "", nil)
	if err != nil {
		t.Fatalf("new: %v", err)
	}
	vm := whatsAppPageVM{
...
func TestNavItemsAreRoutable(t *testing.T) {
	...
	s, err := New(nil, nil, nil, a, &OIDC{}, fakeWhatsAppManager{}, &exechistory.Store{}, "", nil)
	if err != nil {
		t.Fatalf("new: %v", err)
	}
